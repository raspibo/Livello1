...
# Parametri generali
TestoPagina="Start/Stop Daemons Alarms"
DirBase="/var/www"
...
for i in form.keys():
	if i not in form:							# Non ci sara` mai
		...
		for j in form.getlist("start"):
			print ("Start",j,":")
			print (subprocess.call(['/sbin/start-stop-daemon','--start','--verbose', '--background', '--pidfile', '/var/run/setsalarms_d.pid', '--make-pidfile', '--startas', '/var/www/cgi-bin/setsalarms_d.py','--',j]))
	elif i == "stop":
		for j in form.getlist("stop"):
			print ("Stop",j,":")
			print (subprocess.call(['/var/www/cgi-bin/setsalarms_init.d.sh', 'stop', j]))
	else:
		print ("<h3>Cosa e` successo ? Questo e` il valore in form: </h3>",i)

# Genero il file con i "daemons" in esecuzione, come promemoria
os.system("ps -e -o cmd | grep sets:alarm[s] > /var/www/daemons_alarms_running.list")
